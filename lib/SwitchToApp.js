 ...
 *
 * 冷启动: launchPackage 直接启动（首选）
 * 文本切入: recents() 卡片视图 + 文本匹配（fallback）
 *
 * 用法：
...
module.exports = function (runtime, global) {
   ...
   * @param {string} [opts.cardText] - 卡片视图里匹配的文本（文本切入 fallback 用，默认取 pkg 对应 app 名）
  ...
  function switchToApp (opts) {
    ...
    // 冷启动 (首选): launchPackage 直接启动
    log('冷启动 (首选): launchPackage 直接启动')
    try {
    ...
    // 文本切入 (fallback): recents() 打开卡片视图，点击目标卡片
    try {
      log('文本切入 (fallback): recents() 打开卡片视图，查找目标卡片')
      recents()
      sleep(1500)
