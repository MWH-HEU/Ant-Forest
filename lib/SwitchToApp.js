 ...
 *
 * 方法1: launchPackage 直接启动（首选）
 * 方法2: recents() 卡片视图 + 文本匹配（fallback）
 *
 * 用法：
...
module.exports = function (runtime, global) {
   ...
   * @param {string} [opts.cardText] - 卡片视图里匹配的文本（方法2 fallback 用，默认取 pkg 对应 app 名）
  ...
  function switchToApp (opts) {
    ...
    // 方法2 (fallback): recents() 打开卡片视图，点击目标卡片
    try {
      log('方法2 (fallback): recents() 打开卡片视图，查找目标卡片')
      recents()
      sleep(1500)
