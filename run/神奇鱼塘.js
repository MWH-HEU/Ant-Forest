 ...
 * 4. 通过控件循环查找并完成3个任务：
 *    - 浏览商品：入口先处理"做任务领奖励"弹窗，否则循环下滑查找"抵"/"抵后价"商品并点击
...
/**
 * 执行浏览商品任务：入口先判断"做任务领奖励"弹窗，存在则点击后直接返回；
 * 不存在则循环3次"滑动→查找并点击商品→返回列表"，
 * 每次优先匹配"抵"（完全匹配）或"抵后价..."商品并点击；循环结束后再返回一次
...
function doBrowseTask () {
  ...
  // 入口先判断"做任务领奖励"弹窗
  let dlg = widgetUtils.widgetWaiting('做任务领奖励', '做任务领奖励弹窗', 2000)
  if (dlg) {
    taskLog('检测到"做任务领奖励"，点击')
    findAndClickByTextVisible(/做任务领奖励/)
    sleep(2000)
    return
  }
