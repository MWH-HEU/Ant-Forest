...
function step1_openBackpack () {
  ...
  sleep(1000)
  widgetUtils.widgetWaiting('.*(蚂蚁森林|森林|收集能量|浇水|去保护|找能量|森林广场).*', '蚂蚁森林页面', 3000)
  sleep(3000)
  taskLog('蚂蚁森林已打开')
        ...
        try {
          ...
          if (t) {
            ...
            if (text.indexOf('道具') >= 0 || text.indexOf('伙伴') >= 0) {
              ...
              taskLog('找到"' + text + '"，高度: ' + itemHeight + '，点击位置: (' + targetX + ', ' + targetY + ')')
              break
            }
          }
        } catch (e) {}
...
function step3_clickEnergyRainCardExchange () {
      ...
      for (let i = 0; i < allTextViews.size(); i++) {
        ...
        try {
          ...
          if (t && t.toString().indexOf('能量雨次卡') >= 0) {
            let bounds = tv.bounds()
            taskLog('控件找到"能量雨次卡"，点击卡片中心: (' + bounds.centerX() + ', ' + bounds.centerY() + ')')
  ...
  if (!found && localOcrUtil.enabled) {
    ...
    if (screen) {
      ...
      if (results && results.length > 0) {
        let match = results[0]
        taskLog('OCR找到"能量雨次卡"，点击: (' + match.bounds.centerX() + ', ' + match.bounds.centerY() + ')')
        automator.click(match.bounds.centerX(), match.bounds.centerY())
        sleep(500)
        found = true
      }
...
function step5_findAndUseEnergyRainCard () {
  ...
  while (!found && scrollAttempt < maxScrollAttempts) {
    if (scrollAttempt > 0) {
      taskLog('第' + (scrollAttempt + 1) + '次滑动查找')
      // 向上滑动一段距离（背包列表是垂直滚动的）
          ...
          try {
            ...
            if (t) {
              ...
              if (text.indexOf('能量雨机会') >= 0 || text.indexOf('能量雨次卡') >= 0) {
                ...
                taskLog('控件找到"' + text + '"，位置: (' + cardBounds.centerX() + ', ' + cardBounds.centerY() + ')')
      ...
      if (screen) {
        ...
        if (cardResults && cardResults.length > 0) {
          ...
          if (useResults && useResults.length > 0) {
            ...
            taskLog('OCR找到"使用"，点击: (' + match.bounds.centerX() + ', ' + match.bounds.centerY() + ')')
            automator.click(match.bounds.centerX(), match.bounds.centerY())
            sleep(500)
            found = true
          } else {
            ...
            taskLog('尝试点击卡片底部: (' + clickX + ', ' + clickY + ')')
